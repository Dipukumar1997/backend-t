...
export const register = async (req, res) => { // ✅ Fixed: Uncommented and properly wrapped function
    ...
    try {
        ...
        if (existingUser) {
            return res.json({ success: false, message: "user already exist with this email ( in rester in controller in authCOntroller) " });
        }
...
export const login = async (req, res) => {
    ...
    if (!email || !password) {
        return res.json({ success: false, message: "Email and password are required in authcontroller in conroller in backned " });
    }

    try {
        const user = await userModel.findOne({email});
        if (!user) {
            return res.json({ success: false, message: "Invalid email in login function in email" });
        }
